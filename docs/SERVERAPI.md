...
This document describes the API for interacting with the GoChat Server. The server provides functionality to manage client registration, key exchange, message encryption, and various server commands.
...
- Retrieves information about the server, including its IP address(es) and hostname.
- **Response:**
  ```
  SERVERINFO Server: <hostname>
  SERVERINFO IP(s): <ip1>, <ip2>
  ```
...
### Client-to-Client Messages (AES Encryption)
- **Algorithm**: AES (Advanced Encryption Standard) is used for direct client-to-client messages.
- **Key Derivation**: The shared secret obtained from the ECDH key exchange is hashed using **SHA-256** to derive the AES encryption key.
- **Encryption Mode**: AES is used in **CBC (Cipher Block Chaining)** mode.
  - The **IV (Initialization Vector)** is included at the beginning of the ciphertext.
  - Clients must use the derived shared secret and IV to decrypt the client-to-client message.
